...
3. Select and copy the location (e.g. "C:\\\\Users\\\\username\\\\Desktop").
4. Paste the location name inside the setwd() parentheses and add your file name (e.g."\\\\MES623_Lab").


```{r,eval=FALSE}
setwd("/Users/username/Desktop/MES623_Lab")
```
...
# Data: Load, Explore, Manipulate, Export, and Visualize
...
```{r, echo=FALSE}
data = read.csv("MES623_Lab1_mtcars.csv")
```
...
```{r}
ggplot(data = data, mapping = aes(x = wt, y = mpg)) + geom_point()

# Base R
plot(data$wt, data$mpg, pch=19, ylab = "mpg", xlab ="wt") 
```
...
#Base R
plot(data$wt, data$mpg, pch=19, ylab = "mpg", xlab ="wt") 
abline(lm(data$mpg~data$wt), col="slateblue")
```  
...
cyl_4 = data %>% filter(cyl == "4")
cyl_6 = data %>% filter(cyl == "6")
cyl_8 = data %>% filter(cyl == "8")
...
```{r}
ggplot(data, aes(x=as.factor(cyl), y=mpg)) + 
geom_boxplot(fill="slateblue", alpha=0.2) + xlab("cyl")

# Base R
boxplot(mpg~cyl,data=data,xlab="cyl", ylab="mpg") 
```

Attempt a histogram of the data using the `geom_histogram()` function. Change the binwidth to see what happens.
```{r}
ggplot(data, aes(x = mpg)) + geom_histogram(binwidth = 1)

# Base R
hist(data$mpg, breaks=34, col="darkgrey", main = "", xlab="mpg") 
```
